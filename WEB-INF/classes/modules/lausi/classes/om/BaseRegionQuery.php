...
/**
 * Base class that represents a query for the 'lausi_region' table.
 *
 * Barrios
 *
 ...
 *
 * @method     RegionQuery leftJoinAddress($relationAlias = null) Adds a LEFT JOIN clause to the query using the Address relation
...
abstract class BaseRegionQuery extends ModelCriteria
{
	...
	public function filterByAddress($address, $comparison = null)
	{
		if ($address instanceof Address) {
			return $this
				->addUsingAlias(RegionPeer::ID, $address->getRegionid(), $comparison);
	...
	public function filterByClientAddress($clientAddress, $comparison = null)
	{
		if ($clientAddress instanceof ClientAddress) {
			return $this
				->addUsingAlias(RegionPeer::ID, $clientAddress->getRegionid(), $comparison);
