...
abstract class BaseBillboard extends BaseObject  implements Persistent
{
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->height = ($row[$startcol + 3] !== null) ? (boolean) $row[$startcol + 3] : null;
			$this->row = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->billboardcolumn = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			$this->addressid = ($row[$startcol + 6] !== null) ? (int) $row[$startcol + 6] : null;
			$this->resetModified();
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = BillboardPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				BillboardPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(BillboardPeer::ID) ) {
	...
	{
		$pos = BillboardPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Billboard'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Billboard'][$this->getPrimaryKey()] = true;
		$keys = BillboardPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			...
			$keys[4] => $this->getRow(),
			$keys[5] => $this->getBillboardcolumn(),
			$keys[6] => $this->getAddressid(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setBillboardcolumn($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setAddressid($arr[$keys[6]]);
	}
	...
	public function getAddress(PropelPDO $con = null)
	{
		if ($this->aAddress === null && ($this->addressid !== null)) {
			$this->aAddress = AddressQuery::create()->findPk($this->addressid, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aAddress->addBillboards($this);
			 */
		}
	 ...
	 * @return     PropelCollection|array Advertisement[] List of Advertisement objects
	 */
	public function getAdvertisementsJoinWorkforce($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = AdvertisementQuery::create(null, $criteria);
		$query->joinWith('Workforce', $join_behavior);
	...
	{
		return (string) $this->exportTo(BillboardPeer::DEFAULT_STRING_FORMAT);
	}
