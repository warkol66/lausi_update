...
/**
 * Base class that represents a row from the 'lausi_region' table.
 *
 * Barrios
 *
...
abstract class BaseRegion extends BaseObject  implements Persistent
{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(RegionPeer::ID) ) {
	 ...
	 * @return     PropelCollection|array Address[] List of Address objects
	 */
	public function getAddresssJoinCircuit($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = AddressQuery::create(null, $criteria);
