...
abstract class BaseTheme extends BaseObject  implements Persistent
{
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->sheetstotal = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(ThemePeer::ID) ) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setSheetstotal($arr[$keys[5]]);
