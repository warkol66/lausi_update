...
/**
 * Base class that represents a row from the 'lausi_photo' table.
 *
 * Base de Fotos
 *
...
abstract class BasePhoto extends BaseObject  implements Persistent
{
	...
	public function setType($v)
	{
		...
		if ($this->type !== $v) {
			$this->type = $v;
			$this->modifiedColumns[] = PhotoPeer::TYPE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = PhotoPeer::NUM_HYDRATE_COLUMNS.
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PhotoPeer::TYPE)) $criteria->add(PhotoPeer::TYPE, $this->type);
	 ...
	 * @return     PropelCollection|array AddressPhoto[] List of AddressPhoto objects
	 */
	public function getAddressPhotosJoinAddress($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = AddressPhotoQuery::create(null, $criteria);
	 ...
	 *
	 * By default this just sets the collAddresss collection to an empty collection (like clearAddresss());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 ...
	 */
	public function countAddresss($criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collAddresss || null !== $criteria) {
			if ($this->isNew() && null === $this->collAddresss) {
				return 0;
	 ...
	 *
	 * @param      Address $address The AddressPhoto object to relate
	 * @return     void
	 */
	public function addAddress($address)
	{
		...
		if (!$this->collAddresss->contains($address)) { // only add it if the **same** object is not already associated
			$addressPhoto = new AddressPhoto();
			$addressPhoto->setAddress($address);
