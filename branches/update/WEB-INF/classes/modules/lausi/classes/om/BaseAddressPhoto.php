...
/**
 * Base class that represents a row from the 'lausi_addressPhotos' table.
 *
 * 
 *
 * @package    propel.generator.lausi.classes.om
 */
abstract class BaseAddressPhoto extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'AddressPhotoPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        AddressPhotoPeer
	 */
	 ...
	 * @return     AddressPhoto The current object (for fluent API support)
	 */
	public function setAddressid($v)
	{
		...
		if ($this->addressid !== $v) {
			$this->addressid = $v;
			$this->modifiedColumns[] = AddressPhotoPeer::ADDRESSID;
		}
	...
	public function setPhotoid($v)
	{
		...
		if ($this->photoid !== $v) {
			$this->photoid = $v;
			$this->modifiedColumns[] = AddressPhotoPeer::PHOTOID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->photoid = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = AddressPhotoPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating AddressPhoto object", $e);
		}
	}
	...
	public function ensureConsistency()
	{
		...
		if ($this->aPhoto !== null && $this->photoid !== $this->aPhoto->getId()) {
			$this->aPhoto = null;
		}
	} // ensureConsistency
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AddressPhotoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				AddressPhotoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$affectedRows += AddressPhotoPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = AddressPhotoPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = AddressPhotoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['AddressPhoto'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['AddressPhoto'][serialize($this->getPrimaryKey())] = true;
		$keys = AddressPhotoPeer::getFieldNames($keyType);
		$result = array(
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(AddressPhotoPeer::ADDRESSID)) $criteria->add(AddressPhotoPeer::ADDRESSID, $this->addressid);
		if ($this->isColumnModified(AddressPhotoPeer::PHOTOID)) $criteria->add(AddressPhotoPeer::PHOTOID, $this->photoid);
	...
	{
		$criteria = new Criteria(AddressPhotoPeer::DATABASE_NAME);
		$criteria->add(AddressPhotoPeer::ADDRESSID, $this->addressid);
		$criteria->add(AddressPhotoPeer::PHOTOID, $this->photoid);
	...
	{
		return (null === $this->getAddressid()) && (null === $this->getPhotoid());
	}
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of AddressPhoto (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     AddressPhoto Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     AddressPhotoPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new AddressPhotoPeer();
		}
		return self::$peer;
	}
	...
	public function getAddress(PropelPDO $con = null)
	{
		if ($this->aAddress === null && ($this->addressid !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aAddress->addAddressPhotos($this);
			 */
		}
	...
	{
		if ($this->aPhoto === null && ($this->photoid !== null)) {
			$this->aPhoto = PhotoQuery::create()->findPk($this->photoid, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aPhoto->addAddressPhotos($this);
			 */
		}
		return $this->aPhoto;
	}
	...
	{
		return (string) $this->exportTo(AddressPhotoPeer::DEFAULT_STRING_FORMAT);
	}
